...
class PartsManagement(tk.Frame):
    ...
    def display_parts(self):
        ...
        # Include all 12 fields from the database plus Actions column
        headers = ["ID", "Name", "Model", "Size", "Buy", "Qty", "Total", "Sell", "Add Date", "Add Time", "Upd Date", "Upd Time", "Actions"]
        col_widths = [4, 15, 12, 8, 8, 6, 10, 8, 10, 8, 10, 8, 12]  # Adjusted widths for all columns
        ...
        for i, (col, width) in enumerate(zip(headers, col_widths)):
            tk.Label(header_frame, text=col, width=width, bg='#3a7ca5', fg='white', 
                    font=("Segoe UI", 9, "bold"), padx=2, pady=6, anchor='center').pack(side="left")
        ...
        for part in parts:
            ...
            # Display all 12 part data fields with proper formatting and alignment
            for i, (val, width) in enumerate(zip(part, col_widths)):
                if i >= len(col_widths) - 1:  # Stop before the Actions column
                    break
                    
                if i in [4, 6, 7]:  # Format currency fields (Buy, Total, Sell)
                    try:
                ...
                tk.Label(row, text=display_text, width=width, anchor=anchor, 
                        bg=row_color, font=("Segoe UI", 8), fg=fg, padx=2).pack(side="left")
            ...
            tk.Button(action_frame, text="Del", command=lambda p=part: self.remove_part(p),
